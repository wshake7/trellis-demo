...
| EP 2.50.0 误报 `UnrecognisedJavadocTag` 在含中文 javadoc 上  | raw-text regex `\\{@(code                                                           | link)`配`text.indexOf('}')`,多字节字符场景下 byte 位置与 DocTree 解析位置错位,误报"mismatched braces"。EP 源码 `UnrecognisedJavadocTag.java:62` 已知缺陷 | 接受为 false positive；等 EP 2.51+ 修。`grep 'UnrecognisedJavadocTag' warn-report.txt` 5 个 baseline 警告全是这种误报 |

**Phase 1 基线（2026-06-15 收）**：`mvn clean compile 2>warn-report.txt` 共 **7 个 EP warnings**：

- 5× `[UnrecognisedJavadocTag]` 在手写代码 — **EP 2.50.0 false positive**（上述）
- 2× `[EmptyBlockTag]`/`[InvalidBlockTag]` 在 Easy-Query APT 生成的 `SysUserProxy.java` — **Easy-Query 3.2.12 上游 bug**（下个任务回头看）
- 下次升级到 **EP 2.51+** 或 **Easy-Query ≥ 带 `@Generated` 标注的版本** 时回头收敛
